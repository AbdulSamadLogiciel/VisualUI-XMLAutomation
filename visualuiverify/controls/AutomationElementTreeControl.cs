...
//                _elementsTreeView.Nodes[0].Expand();
        ...
        private static List<XElement> CreateXmlElement(AutomationElementTreeNodeCollection treeViewNodes)
        {
            var elements = new List<XElement>();
            
            foreach (AutomationElementTreeNode treeViewNode in treeViewNodes)
            {
                string name = treeViewNode.AutomationElement.Current.LocalizedControlType;
                name = string.Concat(name.Split(null));
                XElement element = new XElement(name);
                element.SetAttributeValue("AutomationID", treeViewNode.AutomationElement.Current.AutomationId);
                element.SetAttributeValue("Name", treeViewNode.AutomationElement.Current.Name);
                element.SetAttributeValue("ControlType", treeViewNode.AutomationElement.Current.ControlType.ProgrammaticName);
                if (treeViewNode.TreeNode.GetNodeCount(true) == 1)
                    //element.Value = treeViewNode.TreeNode.Nodes[0].Tag.ToString();
                    element.Value = treeViewNode.TreeNode.Nodes[0].Text;
                else
                    element.Add(CreateXmlElement(treeViewNode.Children));
        ...
        private void generateXMLMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var rootElement = new XElement(SelectedNode.AutomationElement.Current.LocalizedControlType, CreateXmlElement(SelectedNode.Children));
                var document = new XDocument(rootElement);
               document.Save(@"D:\UIAVerify-master\XMLS\"+"Test.xml");
            }
        }
        ...
        private void goToRootNode(object sender, EventArgs e)
        {
            var rootNode = this.RootNode.TreeNode;
      
            XMLGenerator.GenerateXML(rootNode);
        ...
        private void _elementsTreeView_AfterCheck(object sender, TreeViewEventArgs e)
        {
            var TreeNode = e.Node;

            var listOfNodes = TreeNode.Nodes;
